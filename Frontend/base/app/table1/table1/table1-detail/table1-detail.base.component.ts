...
export class Table1DetailBaseComponent{
 ...
 onChangelog() {
  ...
  const requestedObj = this.generateRequestObj(data);
      ...
      const saveSubscription = this.uploadAttachmentsandSaveData(requestedObj, splittedData).subscribe(res => {
        ...
        const saveSubscription = this.table1Service[method](requestedObj).subscribe((res:Table1Base) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.table1Service.create(splittedData.data).subscribe(
        createdData => {
...
const updateSubscription =  this.saveFiles(splittedData).subscribe((dataToUpdate:any)=>{
  ...
  this.table1Service.update(dataToUpdate.data).subscribe(
   res => {
     subject$.next(res);
     subject$.complete();
   },
...
step:workflowInfo.step
       ...
       const methodName: any = "configureFormOnWorkflow";
        let action: Exclude<keyof Table1DetailBaseComponent, ' '> = methodName;
	...
	editFormula(config:any,fieldname:any){
     ...
     if(fieldConfig.formulaMethod){
       let methodName: Exclude<keyof Table1DetailBaseComponent, ' '> = fieldConfig.formulaMethod
...
if (Array.isArray(ele)) {
  ...
  if (environment.prototype && this.id) {
    ...
    this.table1Service.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const dataSubscription = this.table1Service.getById (params).subscribe((res: Table1Base[]) => {
      this.data = res || {};
      this.formatRawData();
    });
